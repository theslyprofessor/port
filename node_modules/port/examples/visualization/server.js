...
const dir = path.dirname(path.relative(process.cwd(), process.argv[1]));
...
// create a uWebSockets instance to push data to the client
const ws = uWS.App().ws('/*', {
	open: (ws) => {
		// create the port
		const pd = new port({
		...
		ws.on('close', () => pd.destroy());
	}
}).listen(3000, (token) => {
	if (token) {
		console.log('WebSocket server is listening on port 3000');
	} else {
		console.log('WebSocket server failed to listen on port 3000');
	}
});
