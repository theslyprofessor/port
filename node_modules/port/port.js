...
class Port extends EventEmitter {
  ...
  parseFlags(flags) {
    ...
    const path = flags['-path'] || flags['path'] || '';
  ...
  spawn() {
    ...
    const child = this.child = spawn(this.options.pd, this.parseFlags(this.options.flags));
    if (this.options.encoding) child.stderr.setEncoding(this.options.encoding);
    child.on('exit', () => this.emit('exit'));
    child.stderr.on('data', (data) => this.emit('stderr', data));
    return this;
  }

  isRunning() {
    return !!this.child && !this.child.killed && !!this.sender && this.sender.writable;
  }

  listen() {
    const receiver = this.receiver = net.createServer();
    if (this.options.max) receiver.maxConnections = this.options.max;
    receiver.listen(this.options.read, this.options.host);
    receiver.on('listening', () => this.emit('listening'));
    receiver.on('connection', socket => this.connection(socket));
    receiver.on('error', (error) => this.emit('error', error));
  }
  ...
  connect() {
    const sender = this.sender = new net.Socket();
    if (this.options.encoding) sender.setEncoding(this.options.encoding);
    sender.on('connect', () => this.emit('connect', sender));
    sender.on('error', (error) => this.emit('error', error));
